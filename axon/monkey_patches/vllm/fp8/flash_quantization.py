...
#
# Adapted from FlashRL (github.com/LLM360/Flash-RL), Apache-2.0.
import logging
import os
...
def linear_quantize(name, from_p, profile):
    ...
    if name in profile:
        from_p = (
            from_p
            * move_to_device(profile[name]["output_scale"], device)
            * move_to_device(profile[name]["input_scale"], device)
        )
        ...
        for layernorm, layers in input_linear_map.items():
            if layernorm in name:
                ...
                for layer_i in layers:
                    ...
                    dead_neurons_mask_i = (
                        weights[layer_name].abs().float().sum(dim=0, keepdim=False) == 0
                    )  # 1 := needs to be pruned
                    dead_neurons_mask = (
                        dead_neurons_mask * dead_neurons_mask_i
                    )  # to-be-pruned := needs to be pruned for all layers
...
def profiling_fp8(quantized_model, profile_save_to):
    ...
    m = AutoModelForCausalLM.from_pretrained(  # nosec B615
        quantized_model, device_map="cpu", revision=revision
    )
    AutoTokenizer.from_pretrained(quantized_model, revision=revision)  # nosec B615
...
def profiling_int8(model, quantized_model, profile_save_to):
    revision = os.environ.get("HF_HUB_REVISION")
    m = AutoModelForCausalLM.from_pretrained(model, device_map="cpu", revision=revision)  # nosec B615
    qmodel = AutoModelForCausalLM.from_pretrained(  # nosec B615
        quantized_model, device_map="cpu", revision=revision
    )
    AutoTokenizer.from_pretrained(quantized_model, revision=revision)  # nosec B615
        ...
        for balance, smooth in input_linear_map.items():
            if balance in k:
                if smooth is None:
                    ...
                    profile[k] = {
                        "input_scale": 1.0,
                        "output_scale": 1.0 / original_output_scale.float(),
                        "type": qparam[k].data.dtype,
                    }
                else:
