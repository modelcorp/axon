...
from axon.utils.import_utils import is_trl_available
from axon.utils.transformers_compat import is_transformers_version_in_range
from axon.utils.ulysses import (
...
def _ulysses_flash_attention_forward(
    ...
    **kwargs,
    ...
    # ViT calls do not pass position_ids to flash attention, so position_ids is
    # the current model-agnostic guard for applying Ulysses SP here.
    if ulysses_sp_size > 1 and position_ids is not None:
        ...
        # prepare_fa2_from_position_ids needs full position_ids. Passing
        # cu_seq_lens_q, cu_seq_lens_k, max_length_k, and max_length_q directly
        # would avoid this all-gather once that call surface is available.
...
def patch_vlm_for_ulysses_input_slicing(model_class: type):
                ...
                if visual_pos_masks is not None:
                    ...
                    if deepstack_visual_embeds is not None:
                        ...
                        if num_visual_in_shard > 0 and num_visual_before > 0:
                            ...
                            from axon.utils.ulysses import get_ulysses_sequence_parallel_rank
    ...
    # Mark the class so prepare_rmpad_inputs knows this model slices Ulysses SP
    # inputs internally and must NOT be sliced again externally (would
    # double-slice).  See _has_internal_ulysses_sp_slicing in forward_utils.py.
    model_class._axon_ulysses_internal_slice = True
...
def patch_forward_with_backends(
    ...
    if model.config.model_type in ["qwen2_5_vl", "qwen2_vl"]:
        from axon.models.transformers.qwen2_vl import forward_with_torch_backend, forward_with_triton_backend
        ...
        from axon.models.transformers.qwen3_vl import forward_with_torch_backend, forward_with_triton_backend

        forward_with_torch_backend_function = forward_with_torch_backend
        forward_with_triton_backend_function = forward_with_triton_backend
    elif model.config.model_type == "glm4v":
        from axon.models.transformers.glm4v import forward_with_torch_backend, forward_with_triton_backend

        forward_with_torch_backend_function = forward_with_torch_backend
        forward_with_triton_backend_function = forward_with_triton_backend
    elif model.config.model_type == "qwen3_moe":
        from axon.models.transformers.qwen3_moe import forward_with_torch_backend, forward_with_triton_backend

        forward_with_torch_backend_function = forward_with_torch_backend
        forward_with_triton_backend_function = forward_with_triton_backend
    else:
        from axon.models.transformers.dense_common import forward_with_torch_backend, forward_with_triton_backend
    ...
    if fused_kernels_backend == "triton":
        assert use_remove_padding, (
            "Triton fused kernel backend requires use_remove_padding=True. "
            "Set actor.fsdp.use_remove_padding=True or use impl_backend='torch'."
        )
...
def apply_monkey_patch(
    ...
    # VLM models need the multimodal hooks below; text-only families use the
    # standard language-model patch path.
    if model.config.model_type in ["qwen2_5_vl", "qwen2_vl"]:
        ...
        from axon.models.transformers.qwen2_vl import forward_with_normal_backend, qwen2_vl_base_forward
        ...
        if use_remove_padding or ulysses_sp_size > 1:
            from axon.models.transformers.qwen2_vl import qwen2_vl_attn_forward
        ...
        from axon.models.transformers.qwen3_vl import (
        ...
        from axon.models.transformers.glm4v import forward_with_normal_backend, glm4v_base_forward
        ...
        # Step 2: patch attention to support ulysses parallelism
        if use_remove_padding or ulysses_sp_size > 1:
            from axon.models.transformers.glm4v import glm4v_attn_forward
    ...
    elif model.config.model_type == "gemma4":
        # Gemma4 is architecturally multimodal even for text-only tasks.
        # Install a pre-hook to auto-inject mm_token_type_ids (all-zeros)
        # so the causal mask builder works without actual multimodal inputs.
        from axon.models.transformers.gemma4 import patch_gemma4_for_text_only_training

        patch_gemma4_for_text_only_training(model)
        print(f"Patched {model.__class__.__name__} for text-only training (mm_token_type_ids hook)")

    elif model.config.model_type == "kimi_vl":
        if use_remove_padding or ulysses_sp_size > 1:
            # Use the local attention override until upstream transformers exposes
            # the needed remove-padding / Ulysses-SP behavior for this family.
            from axon.models.transformers.kimi_vl import _ulysses_flash_attn_forward
    ...
    # Patch Qwen3.5 GDN layers to pass cu_seqlens through conv1d and
    # chunk_gated_delta_rule, preventing cross-sequence state bleed when
    # use_remove_padding=True packs multiple sequences into one tensor.
    if model.config.model_type in ("qwen3_5", "qwen3_5_moe", "qwen3_5_text", "qwen3_5_moe_text"):
        from axon.monkey_patches.transformers.qwen3_5_gdn import patch_qwen3_5_gdn_attention

        patch_qwen3_5_gdn_attention(model)
    ...
    # Patch MoE expert layers with fused Triton kernels for training
    if use_fused_kernels and model.config.model_type == "qwen3_moe":
        from axon.models.transformers.qwen3_moe import patch_qwen3_moe_with_fused_experts

        patch_qwen3_moe_with_fused_experts(model)
