...
from axon.utils.torch.device import is_npu_available
...
def _custom_flash_attention_forward(
    ...
    **kwargs,
    ...
    if position_ids is not None and query_length != 1 and not (torch.diff(position_ids, dim=-1) >= 0).all():
        ...
        # transformers 5.x resolves the flash-attn kernel lazily inside
        # _flash_attention_forward and raises "Could not find any flash attn
        # implementation" unless attn_implementation is set. This VL text path arrives
        # without one, so pass the installed flash_attn 2.x backend explicitly.
        if is_flash_attn_2_available():
            kwargs["attn_implementation"] = kwargs.get("attn_implementation") or "flash_attention_2"
...
def qwen2_vl_attn_forward(
    ...
    **kwargs,
    ...
    # transformers 5.x: rope_scaling attr removed; mrope_section lives in config.rope_parameters
    mrope_section = (
        self.config.rope_parameters["mrope_section"]
        if hasattr(self.config, "rope_parameters")
        else self.rope_scaling["mrope_section"]
    )
...
def _get_input_embeds(
    ...
    if pixel_values is not None:
        ...
        # transformers 5.x: visual() returns BaseModelOutputWithPooling; extract tensor
