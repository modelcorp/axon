...
#
# MLA adapted from Megatron-Core multi_latent_attention.py (github.com/NVIDIA/Megatron-LM), BSD-3-Clause; DSA follows DeepSeek-V3.2.
"""
GLM-5 Multi-Latent Attention (MLA) implementation for Megatron-Core.

GLM-5 uses a sparse MLA mechanism with:
- Query/KV down-projection and up-projection (LoRA-style compression)
- Absorbed attention (weight absorption into query for efficiency)
- Sparse token selection via a learned indexer
- Custom TileLang CUDA kernels for sparse attention forward/backward
"""
...
from .glm5_ops.indexer import generate_varlen_mask_params, lighting_indexer
from .glm5_ops.sparse_mla import SparseMLA
...
class DSAMLASelfAttention(DSAMultiLatentAttention):
    ...
    def get_absorb_query_key_value_tensors(
        ...
        *,
        ...
        # RoPE fusion is intentionally bypassed on this GLM5 path until the
        # fused kernel matches the gathered MLA layout below.
        ...
        def fuse_rope(q, cu_seqlens, gathered=False):
            ...
            # The fused RoPE kernel expects this contiguous layout; CP and
            # gathered paths differ in how the packed sequence metadata is used.
            if gathered:
    ...
    def set_for_recompute_input_layernorm(self):
        ...
        if hasattr(self.linear_kv_down_proj, "save_original_input"):
            ...
            raise ValueError("layernorm recompute for fp8 with MLASelfAttention needs transformer-engine>=2.6.0dev0.")
...
def build_dsa_self_attn_spec():
    """Build the DSA (Dynamic Sparse Attention) self-attention module spec.

    Returns a ModuleSpec for DSAMLASelfAttention with all required submodules
    (Q/KV projections, indexer weights, etc.). Used by get_glm5_spec to replace
    standard MLA attention in each decoder layer, and by the bridge's
    _model_provider for MTP block spec creation.
    """
...
def get_glm5_spec(config, hf_config, vp_stage=None):
    """Build transformer layer spec with GLM-5 MLA attention.

    Args:
        config: Megatron TransformerConfig (MLATransformerConfig)
        hf_config: HuggingFace model config
        vp_stage: Virtual pipeline stage (optional)

    Returns:
        TransformerBlockSubmodules with MLA attention spec
    """
    ...
    self_attn_module_spec = build_dsa_self_attn_spec()
