...
def _nvtx_range(name: str):
    """NVTX range context manager that is a no-op on ROCm (where NVTX is unavailable)."""
    try:
        ctx = torch.cuda.nvtx.range(name)
        with ctx:
            yield
...
class LinearCrossEntropy(torch.autograd.Function):
    ...
    def forward(
        ...
        with _nvtx_range("LinearCrossEntropy-forward"):
            from . import kernels
    ...
    def backward(ctx, dlogprobs: torch.Tensor, dentropy: torch.Tensor) -> list[torch.Tensor]:
        ...
        with _nvtx_range("LinearCrossEntropy-backward"):
