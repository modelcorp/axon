...
def update_model_config(module_config, override_config_kwargs):
    """
    Recursively update a module config with override values.

    Supports both objects with attributes and nested dicts.
    Creates nested dicts if an attribute is None or missing.
    """
    for key, val in override_config_kwargs.items():
        if isinstance(val, dict):
            # Determine the current sub_config
            sub_config = (
                getattr(module_config, key, None)
                if not isinstance(module_config, dict)
                else module_config.get(key, None)
            )
            ...
            if sub_config is None:
                ...
                if isinstance(module_config, dict):
                    module_config[key] = sub_config
                else:
                    setattr(module_config, key, sub_config)

            # Recurse
            update_model_config(sub_config, val)
        else:
            # Set value for dict or object
            if isinstance(module_config, dict):
...
def get_huggingface_actor_config(model_name: str, override_config_kwargs=None, trust_remote_code=False) -> dict:
    ...
    module_config = AutoConfig.from_pretrained(model_name, trust_remote_code=trust_remote_code)  # nosec B615
...
def get_generation_config(
    ...
    try:
        return GenerationConfig.from_pretrained(model)  # nosec B615
...
def print_model_size(model: nn.Module, name: str = None):
    ...
    logger.info("%s contains %.2f%s parameters", name, n_params, scale)
...
def create_random_mask(
    ...
    # This path prioritizes deterministic mask construction over vectorized speed.
...
def convert_param_name(name: str, model=None, mapping: dict | None = None) -> str:
    """Convert a single parameter name from internal format to checkpoint format.

    Applies the reverse of ``_checkpoint_conversion_mapping`` (same logic as
    ``convert_weight_keys`` but for a single name string).

    Args:
        name: Parameter name in internal format.
        model: Model instance with ``_checkpoint_conversion_mapping`` attribute.
        mapping: Explicit ``_checkpoint_conversion_mapping`` dict (takes precedence).
    """
    if mapping is None:
        if model is None or not hasattr(model, "_checkpoint_conversion_mapping"):
            return name
...
def get_checkpoint_conversion_mapping(model_path: str, trust_remote_code: bool = False) -> dict | None:
    """Get ``_checkpoint_conversion_mapping`` from the HF model class for a given model path.

    Returns ``None`` if the model class does not define a mapping.
    """
    try:
        from transformers import AutoConfig

        config = AutoConfig.from_pretrained(model_path, trust_remote_code=trust_remote_code)  # nosec B615
        if not getattr(config, "architectures", None):
            return None
        model_type = getattr(config, "model_type", "")
        module = __import__(
            f"transformers.models.{model_type}.modeling_{model_type}",
            fromlist=[config.architectures[0]],
        )
        model_cls = getattr(module, config.architectures[0], None)
        return getattr(model_cls, "_checkpoint_conversion_mapping", None)
    except Exception as e:
        logger.warning("Failed to load _checkpoint_conversion_mapping for %s: %s", model_path, e)
        return None
...
def check_target_modules(config, key: str) -> bool:
    ...
    if isinstance(config.target_modules, str):
        ...
        if is_using_layer_indexes and target_module_found:
            layer_index = None
            # Empty layers_pattern values intentionally match any layer pattern.
...
def normalize_model_name(name, pp_rank, vpp_rank, transformer_config, layer_name="layers"):
    ...
    """
    from axon.utils.megatron.utils import get_transformer_layer_offset
...
def normalize_pp_vpp_params(params, num_hidden_layers, layer_name="layers"):
    ...
    for pp_rank in range(len(params)):
        ...
        # Build a minimal config object compatible with normalize_model_name / get_transformer_layer_offset
        ...
        if hasattr(transformer_config, "pipeline_model_parallel_layout"):
            delattr(transformer_config, "pipeline_model_parallel_layout")
        for vpp_rank in range(vpp_size):
            for name, param in params[pp_rank][vpp_rank].items():
                normalized_name = normalize_model_name(
                    name, pp_rank, vpp_rank, transformer_config, layer_name=layer_name
                )
...
def load_mcore_dist_weights(parallel_model, dist_weight_path, is_value_model=False, prefix=""):
    ...
    loading_state_dict = {}
    for vpp_rank, model in enumerate(parallel_model):
        ...
        if len(parallel_model) > 1:
            mpu.set_virtual_pipeline_model_parallel_rank(vpp_rank)
            key = f"model{vpp_rank}" if len(parallel_model) > 1 else "model"
        else:
            key = "model"
        loading_state_dict[key] = ssd

    from megatron.core.dist_checkpointing.strategies.tensorstore import (
        TensorStoreLoadShardedStrategy,
    )

    ts_strategy = TensorStoreLoadShardedStrategy(
...
def get_vl_model_vision_tower(vl_model_instance):
    """Extract Vision Tower from a VL model instance."""
...
def apply_lora_to_module(module, lora_cfg, role):
    """Apply LoRA (from a pre-trained adapter or fresh config) to *module*.

    *lora_cfg* is the ``lora:`` sub-config (e.g. ``config.lora``).
    Returns the (possibly wrapped) module.
    """
    from peft import LoraConfig, TaskType, get_peft_model

    from axon.utils.py_utils import convert_to_regular_types

    logger.info("Applying LoRA to %s module", role)
    module.enable_input_require_grads()

    lora_adapter_path = lora_cfg.get("adapter_path")
    if lora_adapter_path is not None:
        from peft import PeftModel

        logger.info("Loading pre-trained LoRA adapter to %s from: %s", role, lora_adapter_path)
        module = PeftModel.from_pretrained(module, lora_adapter_path, is_trainable=True)
        peft_config = module.peft_config["default"]
        if isinstance(peft_config.task_type, str):
            peft_config.task_type = TaskType.CAUSAL_LM
    else:
        target_modules = convert_to_regular_types(lora_cfg.target_modules)
