...
def log_seqlen_unbalance(seqlen_list: list[int], partitions: list[list[int]], prefix):
    ...
    batch_size = math.ceil(len(seqlen_list) / k_partition)
    min_sum_seqlen = None
    max_sum_seqlen = None
    total_sum_seqlen = 0
...
def _max_partition_tokens(partitions: list[list[int]], seq_lens: list[int]) -> int:
    """Return the maximum total-token count across all partitions."""
    return max(sum(seq_lens[i] for i in part) for part in partitions)
...
def rearrange_micro_batches(
    ...
    """
    Split a batch into micro-batches with a hard token-count cap per micro-batch.

    Partitions are balanced by computational workload (quadratic in seq length)
    for efficiency, then **verified** against ``max_token_len`` (total actual
    tokens).  If any partition exceeds the cap the split count is increased and
    the partitioning is retried — typically 0–2 retries.

    ``max_token_len`` is enforced as a hard bound: no micro-batch will contain
    more actual tokens than this value.

    Args:
        batch (TensorDict): must include "attention_mask" (B*S); other fields are sliced similarly.
        max_token_len (int): hard cap on the sum of attention_mask per micro-batch.
    ...
    # Move to CPU once — all subsequent work is CPU-side.
    seq_lens = seq_len_effective.cpu().tolist()
    batch_size = len(seq_lens)
    max_actual_seq = max(seq_lens)
    total_seqlen = sum(seq_lens)

    assert max_token_len >= max_actual_seq, (
        f"max_token_len_per_gpu ({max_token_len}) must be >= the longest actual sequence ({max_actual_seq}). "
        f"A single row must fit in one micro-batch."
    )

    # Initial estimate: enough micro-batches so the *average* fits.
    num_micro_batches = min(batch_size, ceildiv(total_seqlen, max_token_len))
    if min_num_micro_batch is not None:
        num_micro_batches = max(min_num_micro_batch, num_micro_batches)

    # Workload (quadratic) for compute-balanced partitioning.
    workloads = calculate_workload(seq_len_effective).cpu().tolist()

    # Partition by workload, then verify the hard token cap.
    # KK balances compute but can overshoot the token budget.
    # When that happens, increase K and re-partition (fast — CPU only).
    micro_bsz_idx = get_seqlen_balanced_partitions(workloads, num_micro_batches, equal_size=False)

    while _max_partition_tokens(micro_bsz_idx, seq_lens) > max_token_len:
        num_micro_batches += 1
        assert num_micro_batches <= batch_size, (
            f"Cannot satisfy max_token_len={max_token_len} even with 1 row per micro-batch. "
            f"This should be impossible given the assertion above."
        )
        micro_bsz_idx = get_seqlen_balanced_partitions(workloads, num_micro_batches, equal_size=False)

    # DP sync: all ranks must process the same number of micro-batches
    # so FSDP gradient all-reduce doesn't deadlock.
    if dist.is_initialized() and same_micro_num_in_dp:
        num_micro_batches = torch.tensor([num_micro_batches], device=get_device_name())
        dist.all_reduce(num_micro_batches, op=dist.ReduceOp.MAX, group=dp_group)
        num_micro_batches = int(num_micro_batches.cpu().item())

    if num_batches_divided_by is not None:
        num_micro_batches = roundup_divisible(num_micro_batches, num_batches_divided_by)

    assert num_micro_batches <= batch_size, f"num_micro_batches ({num_micro_batches}) > batch_size ({batch_size})."

    # Re-partition if the count changed (allreduce or divisibility bump).
    # More partitions → smaller groups, so the token cap is still satisfied.
    if num_micro_batches != len(micro_bsz_idx):
        micro_bsz_idx = get_seqlen_balanced_partitions(workloads, num_micro_batches, equal_size=False)
        # Safety check: verify token cap still holds after re-partition.
        max_part = _max_partition_tokens(micro_bsz_idx, seq_lens)
        if max_part > max_token_len:
            logger.warning(
                "Post-sync re-partition exceeds token cap (%d > %d). Falling back to token-based partitioning.",
                max_part,
                max_token_len,
            )
            micro_bsz_idx = get_seqlen_balanced_partitions(seq_lens, num_micro_batches, equal_size=False)

    if use_dynamic_bsz_balance:
        # Reorder micro-batches by workload for pipeline efficiency.
        micro_bsz_idx.sort(
        ...
        # Place smaller micro-batches at both ends to reduce bubbles during warm-up and cool-down.
