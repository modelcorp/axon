...
class Profiler:
    ...
    This wrapper is intentionally local to the PyTorch profiler path; distributed
    profiler integration can be layered on top of the same config objects.
    ...
    def __init__(self, config: ProfilerConfig, tool_config: TorchProfilerToolConfig | None = None):
        ...
        if self.rank in self.config.profile_ranks:
            logger.info("[Profiler] Profiler init for rank %d", self.rank)
    ...
    def _validate(self):
        if self.enable:
            if self.config.profile_ranks is None:
                logger.warning("Profile ranks is not set, default to rank 0")
    ...
    def save(self):
        if self.prof is not None and not self.saved:
            ...
            logger.info("[Profiler] Saving trace to %s", self.config.save_path + save_file_name)
    ...
    def stop_trace(self):
        if self.check():
            logger.info("[Profiler] Trace stopped for rank %d", self.rank)
            self.enable = False
...
class DistProfiler:
    ...
    def __init__(self, rank: int, config: ProfilerConfig | None = None, tool_config: object | None = None, **kwargs):
        # Default config
        if not config:
            config = ProfilerConfig(ranks=[], enable=False)
...
class TorchMemoryProfiler:
    ...
    def __init__(self, rank: int, config: ProfilerConfig | None, tool_config: TorchMemoryToolConfig | None = None):
...
class DistProfilerExtension:
    ...
    It is intended for workers in axon that single controller invokes.
