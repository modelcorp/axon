...
def _apply_logit_softcapping(logits: torch.Tensor, softcap: float | None) -> torch.Tensor:
    """Apply Gemma-style logit soft-capping: tanh(logits / cap) * cap."""
...
def _fused_linear_for_ppo_fwd(
    ...
    logits = _apply_logit_softcapping(logits, logit_softcapping)
...
def _fused_linear_for_ppo_bwd(
    ...
    raw_logits = hidden_states @ vocab_weights.t()
    logits = _apply_logit_softcapping(raw_logits, logit_softcapping)
    logits = logits / temperature
    orig_dtype = logits.dtype
    logits = logits.to(torch.float32)
    ...
    # Chain rule through softcapping: d(tanh(x/cap)*cap)/dx = 1 - tanh²(x/cap)
    if logit_softcapping is not None:
        softcap_grad = 1.0 - (torch.tanh(raw_logits / logit_softcapping)) ** 2
        dlogits = dlogits * softcap_grad.to(dlogits.dtype)
