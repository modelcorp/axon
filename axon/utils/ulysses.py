...
def ulysses_pad_and_slice_inputs(
    ...
    Args:
        ...
        skip_position_ids_rmpad: whether to skip position_ids_rmpad.
...
def build_ulysses_device_mesh(world_size, sp_size):
    """Build a device mesh for Ulysses Sequence Parallel, or None if sp_size <= 1."""
    if sp_size > 1:
        ...
        return init_device_mesh(device_name, mesh_shape=(dp, sp_size), mesh_dim_names=["dp", "sp"])
    return None


def register_ulysses_dispatch(worker, mesh_name, ulysses_mesh):
    """Register dispatch/collect info based on Ulysses mesh or plain rank."""
    if ulysses_mesh is not None:
        is_collect = ulysses_mesh["sp"].get_local_rank() == 0
        worker._register_dispatch_collect_info(
            mesh_name, dp_rank=ulysses_mesh["dp"].get_local_rank(), is_collect=is_collect
        )
    else:
        worker._register_dispatch_collect_info(mesh_name, dp_rank=worker.rank, is_collect=True)
...
class FSDPUlyssesShardingManager:
    ...
    def preprocess_data(self, data):
        ...
        """
        if self.device_mesh is not None:
            from axon.protocol import all_gather_data_proto
