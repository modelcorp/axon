...
from axon.utils.fsdp.utils import FSDPModule as FSDP2
...
class AsyncDoubleBufferGroupOffloadHandler(SynchronizedGroupOffloadHandler):
    ...
    def tensor_pop(self, tensor_tag, **kwargs):
        ...
        # The tensor should have been reloaded by bulk_reload_group() via
        # on_group_commit_backward(). If it's still a (key, shape) tuple (e.g.
        # because gradient checkpointing reorders the backward calls relative to
        # group-commit callbacks), fall back to a synchronous CPU→GPU reload.
        if isinstance(tensor, tuple):
            key, shape = tensor
            group_id = tensor_tag[0]
            # Wait for any in-flight D2H transfers before synchronous reload
            get_torch_device().current_stream().wait_stream(self.d2h_stream)
            cpu_state = self.group_offload_mapping[group_id][key]
            tensor = SynchronizedGroupOffloadHandler.reload(cpu_state, non_blocking=False).view(shape)
        else:
            # Tensor was bulk-reloaded on h2d_stream by a previous
            # on_group_commit_backward().  With gradient checkpointing the
            # backward call order may differ from the group-commit order,
            # so tensor_pop can race ahead of the commit that waits for
            # h2d_stream.  Ensure the compute stream sees the completed
            # H2D transfer before using the tensor.
            group_id = tensor_tag[0]
            if group_id < self.num_offload_group:
    ...
    def on_group_commit_backward(self):
        ...
        # Guard against offloaded_group_count == 0 (e.g. MoE models where extra
        # autograd group commits in backward have no matching forward offload).
        if (
            self.offloaded_group_count > 0
            ...
            self.offloaded_group_count = max(0, self.offloaded_group_count - 1)
...
class ActivationHandler:
    ...
    def pre_forward(self, module):
        if module.training:
            ...
            if not self._enable_ckpt:
                # Without checkpointing, push offload hooks around the whole
                # forward so ALL saved tensors are offloaded.
                self._offload_ctx.__enter__()

    def post_forward(self, module):
        if module.training and not self._enable_ckpt:
    ...
    def _ckpt_forward(self, forward_method, *args, **kwargs):
        ...
        offload_ctx = self._offload_ctx

        def my_function(*inputs):
            # Push offload hooks INSIDE the checkpoint body.
            # With use_reentrant=True the first forward runs under no_grad
            # (hooks don't fire), but the backward re-run is under
            # enable_grad — hooks fire and activations are offloaded.
            #
            # Crucially this is INSIDE the checkpoint, so the hooks do NOT
            # intercept the checkpoint's own saved inputs (which must stay
            # on GPU for the backward re-run to work).
            nonlocal forward_method, kwarg_keys, offload_ctx
        ...
        for module in model.modules():
            if hasattr(module, "gradient_checkpointing_disable"):
                try:
                    ...
                    # Some sub-models (e.g. Gemma4AudioModel inside the multimodal
                    # Gemma4ForConditionalGeneration) advertise ``supports_gradient_checkpointing``
                    # at the class level but have no submodule with the ``gradient_checkpointing``
                    # attribute, so HF raises ValueError on disable.  Swallow it: there's no
                    # checkpointing to turn off in that branch anyway.
                    pass
