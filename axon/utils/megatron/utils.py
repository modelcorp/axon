...
import axon.utils.megatron.tensor_parallel as tp_utils
from axon.utils.fs import local_mkdir_safe
from axon.utils.hf_model import normalize_model_name
...
def make_megatron_module(
    ...
    if bridge is not None:
        if provider is None:
            from axon.models.mbridge import freeze_moe_router as freeze_moe_router_fn
            from axon.models.mbridge import make_value_model

            value_model_hook = make_value_model
        else:
            from axon.models.megatron_bridge import freeze_moe_router as freeze_moe_router_fn
            from axon.models.megatron_bridge import make_value_model
        ...
        if freeze_moe_router:
            post_model_creation_callbacks.append(freeze_moe_router_fn)
        if provider is not None:
            ...
            if peft_cls is not None:
                from axon.utils.megatron.peft_utils import load_adapter_checkpoint, print_adapter_info
        ...
        raise AssertionError("mbridge is required. Non-bridge model initialization is no longer supported.")
...
def convert_config(hf_config: PretrainedConfig, megatron_config) -> TransformerConfig:
    ...
    logger.info("megatron config %s", megatron_config)
    dt = PrecisionType.to_dtype(megatron_config.params_dtype)
    logger.info("pipeline_dtype=megatron_config %s", dt)
...
def mcore_model_parallel_config(
    ...
    # Please use hf_to_mcore_config_dense() from megatron-bridge instead.
    warnings.warn(
        "Code should not reach this point. This function is deprecated and will be removed. Please use "
        "hf_to_mcore_config_dense() from megatron-bridge instead.",
        DeprecationWarning,
        stacklevel=2,
    )
...
def offload_megatron_model_to_cpu(models):
    ...
    """
    # Ensure all async GPU→CPU copies from previous load have completed
    # before freeing/reusing buffers.
            ...
            for buffers in model_chunk_all_buffers:
                for buffer in buffers:
                    ...
                    if buffer.param_data.storage().size() > 0:
                        # Reuse the pinned CPU buffer across offload cycles to avoid
                        # CUDA host memory fragmentation from repeated pin_memory() calls.
                        if hasattr(buffer.param_data, "cpu_data") and buffer.param_data.cpu_data is not None:
...
def convert_megatron_model_to_transformers_model(
    ...
    def convert_qkv_shard(full_tensor, q_name, k_name, v_name):
        ...
        if config.num_key_value_heads >= tp_size:
            ...
            total_size = q_size_tp + 2 * kv_size_tp
            for i in range(tp_size):
                num_query_groups_per_partition = max(1, num_query_groups // tp_size)
                qkv_part = full_tensor[i * total_size : (i + 1) * total_size]
                q_size_chunk = q_size_tp // num_query_groups_per_partition
                kv_size_chunk = kv_size_tp // num_query_groups_per_partition
                for qkv_part_chunk in qkv_part.chunk(num_query_groups_per_partition):
...
def default_tp_concat_fn(
    ...
    Current implementation handles the model families wired into the bridge.
    A future model-owned conversion hook would make this path fully model-agnostic;
    if no hook exists, TP HybridEngine should be disabled for that family.
    """
...
def detach_with_attributes(tensor):
    """Safely detach while preserving attributes"""
    detached = tensor.detach()

    # Only try to access attributes that are safe
    safe_attrs = []
    for attr_name in dir(tensor):
        if not attr_name.startswith("_"):
            try:
                # Check if attribute exists and is accessible without calling it
                if hasattr(tensor, attr_name):
                    descriptor = getattr(type(tensor), attr_name, None)
                    # Skip properties and methods
                    if not isinstance(descriptor, property) and not callable(descriptor):
                        safe_attrs.append(attr_name)
            except Exception:
                continue
    # Now safely copy the safe attributes
    for attr_name in safe_attrs:
        try:
            attr_value = getattr(tensor, attr_name)
            if not callable(attr_value):
                setattr(detached, attr_name, attr_value)
        except Exception:
            continue
    return detached
    ...
    def tensor_generator():
        for scan_vpp_idx in range(vpp_size):
            ...
            for name, param in model.named_parameters():
                ...
                yield name, detach_with_attributes(param)
            # note
            ...
            for name in extra_keys:
                yield name, detach_with_attributes(model.state_dict()[name].to(get_device_id()))
        ...
        if isinstance(model[0], megatron_FSDP | DDP) and overlap_grad_reduce:
            ...
            if not is_rocm():
                assert os.environ.get("CUDA_DEVICE_MAX_CONNECTIONS") == "1", (
                    "overlap_grad_reduce requires CUDA_DEVICE_MAX_CONNECTIONS=1. "
                    "Set this env var or disable overlap_grad_reduce."
                )
...
########################## MOE #########################################################
def get_moe_layer_distribution(actor_module, num_vpp_stages=1):
    """
    Get the distribution of MoE layers across pipeline parallel ranks.
    Returns a mapping from rank to MoE layer indices in the compressed routermap.

    Returns:
        Dict[int, List[int]]: Mapping from PP rank to MoE layer indices in routermap
    """
    pp_rank = mpu.get_pipeline_model_parallel_rank()
    pp_world_size = mpu.get_pipeline_model_parallel_world_size()

    # Get MoE layer counts per VPP stage
    local_vpp_moe_counts = count_local_moe_layers(actor_module, num_vpp_stages)  # Returns a list
    local_num_vpp_stages = len(local_vpp_moe_counts)

    # First, gather the number of VPP stages from all ranks to find the maximum
    _dev = f"{get_device_name()}:{get_device_id()}"
    local_vpp_count_tensor = torch.tensor([local_num_vpp_stages], dtype=torch.int32, device=_dev)
    all_vpp_stage_counts = [torch.zeros(1, dtype=torch.int32, device=_dev) for _ in range(pp_world_size)]

    torch.distributed.all_gather(
        all_vpp_stage_counts, local_vpp_count_tensor, group=mpu.get_pipeline_model_parallel_group()
    )

    # Find the maximum number of VPP stages across all ranks
    max_vpp_stages = max(count[0].item() for count in all_vpp_stage_counts)

    # Pad local VPP MoE counts to match the maximum VPP stages
    padded_local_counts = local_vpp_moe_counts + [0] * (max_vpp_stages - local_num_vpp_stages)

    # Convert to tensor for gathering
    local_count_tensor = torch.tensor(padded_local_counts, dtype=torch.int32, device=_dev)

    # Gather padded VPP MoE counts from all ranks
    all_vpp_counts = [torch.zeros(max_vpp_stages, dtype=torch.int32, device=_dev) for _ in range(pp_world_size)]

    torch.distributed.all_gather(all_vpp_counts, local_count_tensor, group=mpu.get_pipeline_model_parallel_group())

    # Build the distribution mapping
    moe_layer_distribution = {}
    current_moe_idx = 0

    for rank in range(pp_world_size):
        # Get actual number of VPP stages for this rank
        rank_num_vpp_stages = all_vpp_stage_counts[rank][0].item()
        # Only use the valid VPP counts (not the padded zeros)
        rank_vpp_counts = all_vpp_counts[rank][:rank_num_vpp_stages]
        rank_total_moe_count = rank_vpp_counts.sum().item()

        if rank_total_moe_count > 0:
            # Create list of MoE layer indices for this rank
            rank_moe_indices = []
            for vpp_idx in range(rank_num_vpp_stages):
                vpp_count = rank_vpp_counts[vpp_idx].item()
                if vpp_count > 0:
                    # Add indices for this VPP stage
                    rank_moe_indices.extend(range(current_moe_idx, current_moe_idx + vpp_count))
                    current_moe_idx += vpp_count

            moe_layer_distribution[rank] = rank_moe_indices
        else:
            moe_layer_distribution[rank] = []

    # Optional: Store VPP-level information for debugging or later use
    # self.vpp_moe_counts = {}
    # for rank in range(pp_world_size):
    #     rank_num_vpp_stages = all_vpp_stage_counts[rank][0].item()
    #     self.vpp_moe_counts[rank] = all_vpp_counts[rank][:rank_num_vpp_stages].tolist()
    total_decoder_moe_layers = current_moe_idx

    return moe_layer_distribution, all_vpp_counts[pp_rank], total_decoder_moe_layers


def count_local_moe_layers(actor_module, num_vpp_stages=1):
    """
    Count the number of MoE layers in each VPP stage of the current pipeline parallel rank.

    Returns:
        List[int]: List where each element is the MoE layer count for a VPP stage
    """
    # Initialize list to store MoE counts per VPP stage
    vpp_moe_counts = []

    # Check if using virtual pipeline parallelism
    if num_vpp_stages == 1:
        # No VPP, treat as single stage
        vpp_moe_counts = [0]
    else:
        # Initialize counts for each VPP stage
        vpp_moe_counts = [0] * num_vpp_stages

    # Iterate through model modules (assuming actor_module contains VPP stages)
    for vpp_idx, module in enumerate(actor_module):
        if vpp_idx >= len(vpp_moe_counts):
            # Safety check in case of mismatch
            vpp_moe_counts.append(0)
        ...
        # Store count for this VPP stage
        if num_vpp_stages == 1:
            vpp_moe_counts[0] += moe_count
        else:
            vpp_moe_counts[vpp_idx] = moe_count

    return vpp_moe_counts
...
def is_moe_layer(layer):
    ...
    Args:
        layer: A model layer module

    Returns:
        bool: True if the layer is an MoE layer
    """
    # Standard Megatron MoE: layer.mlp is an MoELayer with a router
    if hasattr(layer, "mlp") and hasattr(layer.mlp, "router"):
        return True
    # Gemma4 MoE: dense MLP + parallel MoE block at layer.moe_block
