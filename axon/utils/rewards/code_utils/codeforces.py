...
#
# Adapted from hendrycks/apps testing_util.py (github.com/hendrycks/apps), MIT.
...
from axon.utils.rewards.code_utils.pyext2 import RuntimeModule
        ...
        if not method_func:
            ...
            if which_type == CODE_TYPE.call_based:  # Call-based
                detail_results, debug_infos = execute_cb_code(
                    method_func, inputs_list, outputs_list, timeout=timeout, early_stop=True, debug=debug
                )
            elif which_type == CODE_TYPE.standard_input:
                detail_results = execute_std_code(
                    method_func, exec_code, inputs_list, outputs_list, timeout=timeout, early_stop=True, debug=debug
                )
    ...
    for i, inputs in enumerate(inputs_list):
        ...
        try:
            result = subprocess.run(
                ["python", temp_program_path], input=inputs, text=True, capture_output=True, timeout=timeout
            )
            exec_code = 999
