...
# ============================================================================
# Defaults mirror openpipe/art's 2048 example where possible:
#   - WINNING_VALUE   = 128      (ART constant in examples/2048/utils.py)
#   - max_seq_length  = 8192     (ART InitArgs)
#   - learning_rate   = 1e-5     (ART backend.train kwarg)
#   - no max_turns    in ART; we use 128 as a safety cap (games to 128 rarely
#                               exceed ~30 moves)
#   - rollouts/step   = 18       (ART's SIMULTANEOUS_GAMES). Axon uses
#                               train_batch_size * decoding.n instead; see
#                               TRAIN_BATCH_SIZE / DECODING_N below.
#   - base_model      = Qwen2.5-3B-Instruct. This script defaults to the
#                       larger Qwen3-30B-A3B MoE (matching the frozenlake
#                       30B-A3B recipe); override MODEL_PATH if you want the
#                       3B base ART uses (you'd also drop the MoE megatron args).
# Override any of these at the command line, e.g.
#   TARGET_VALUE=512 MAX_TURNS=256 bash train_2048_qwen_30b_a3b.sh
# ============================================================================
...
MAX_SEQ_LENGTH=${MAX_SEQ_LENGTH:-8192}          # ART: 8192
MODEL_PATH=${MODEL_PATH:-Qwen/Qwen3-30B-A3B}    # ART uses Qwen/Qwen2.5-3B-Instruct
ENABLE_PARTIAL_ROLLOUT=False
HYBRID_ENGINE=True
STEP_MODE=False # Enable step-level mode (non-cumulative context)

# 2048 env knobs
TARGET_VALUE=${TARGET_VALUE:-128}               # ART: WINNING_VALUE=128
MAX_TURNS=${MAX_TURNS:-128}                     # ART: no cap; 128 is a safe upper bound
BOARD_SIZE=${BOARD_SIZE:-4}                     # ART: 4x4

# Optimization
LEARNING_RATE=${LEARNING_RATE:-1e-5}            # ART: 1e-5 (for 30B, consider 1e-6)
TRAIN_BATCH_SIZE=${TRAIN_BATCH_SIZE:-64}        # ART: 18 rollouts/step; Axon scales via batch * n
DECODING_N=${DECODING_N:-8}                     # ART implicitly 1 per game (+RULER group-relative)
...
# Default configuration for Qwen3-30B-A3B MoE (matches recipes/frozenlake/train_frozenlake_qwen_30b_a3b.sh)
...
if [ "$LOSS_MODE" = "gspo" ]; then
    ...
    EXPERIMENT_NAME=${MODEL_NAME}-2048_agent-gspo-megatron
    ...
    EXPERIMENT_NAME=${MODEL_NAME}-2048_agent-grpo-megatron
    USE_SAMPLER_LOGPROBS=False
fi
...
python3 -m axon.driver.train_agent_ppo \
    ...
    strategy=megatron \
    advantage=loop \
    train_files=${AXON_DIR}/data/2048/train.parquet \
    val_files=${AXON_DIR}/data/2048/test.parquet \
    ...
    max_steps=${MAX_TURNS} \
    program.name=react \
    +program.env_name=${SCRIPT_DIR}/env.py:Game2048Env \
    +program.env_args.max_turns=${MAX_TURNS} \
    +program.env_args.target_value=${TARGET_VALUE} \
    +program.env_args.size=${BOARD_SIZE} \
    +program.agent_name=${SCRIPT_DIR}/agent.py:Game2048Agent \
