...
LOSS_MODE=ppo # gspo, grpo

ENGINE=vllm # vllm, sglang
MAX_PROMPT_LENGTH=1024
MAX_SEQ_LENGTH=4096
MODEL_PATH=PrimeIntellect/Qwen3-1.7B-Wordle-SFT
...
SAMPLER_TENSOR_MODEL_PARALLEL_SIZE=1
NUM_NODES=1
...
if [ "$LOSS_MODE" = "gspo" ]; then
    ...
    EXPERIMENT_NAME=${MODEL_NAME}-wordle_agent-gspo-megatron
    ...
    BATCH_REDUCE=step-mean
    EXPERIMENT_NAME=${MODEL_NAME}-wordle_agent-grpo-megatron
    USE_SAMPLER_LOGPROBS=False
fi
...
python3 -m axon.driver.train_agent_ppo \
    ...
    strategy=megatron \
    advantage=loop \
    train_files=${AXON_DIR}/data/verifiers/wordle/train.parquet \
    val_files=${AXON_DIR}/data/verifiers/wordle/test.parquet \
    ...
    program.name=verifiers \
    +program.env_module=wordle \
    +program.sampling_params={}
