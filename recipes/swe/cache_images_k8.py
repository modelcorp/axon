...
def pull_image_on_all_nodes(docker_image, total_targets):
    ...
    try:
        # write the DaemonSet to a temporary file
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as tmp:
            yaml.safe_dump(create_daemonset_yaml(docker_image, ds_name), tmp)
            yaml_file = tmp.name
        ...
        # wait for it
        print(f"[wait] sampler status daemonset/{ds_name} (timeout 5m)")
        subprocess.run(
            ["kubectl", "sampler", "status", f"daemonset/{ds_name}", "--timeout=3600s"],
            check=True,
        )
...
with concurrent.futures.ThreadPoolExecutor(max_workers=48) as executor:
    ...
    future_to_image = {
        executor.submit(pull_image_on_all_nodes, image, len(unique_images)): image for image in unique_images
    }
