...
# Start ALL NeMo Gym resource server on other terminal by
# cat > env.yaml << 'EOF'
# hf_token: YOUR_HF_TOKEN
# policy_base_url: http://AXON_HOST:8000/v1
# policy_api_key: not-needed
# policy_model_name: axon
# EOF

# # Start all servers (agent + model config + resource)
...
python3 -m axon.driver.train_agent_ppo \
    ...
    engine_endpoint.host=0.0.0.0 \
    engine_endpoint.port=8000 \
    terminate_on_error=False \
    program.name=nemo_gym \
    +program.mode=native \
    +program.head_server_url=http://localhost:11000 \
    +program.agent_server_name=simple_agent \
    +program.native_timeout=300 \
    max_concurrency=64