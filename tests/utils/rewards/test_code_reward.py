...
from axon.utils.rewards import RewardConfig, RewardType
from axon.utils.rewards.code_reward import RewardCodeFn
...
"""
    ...
    def test_reward_kodcode(self):
        ...
        Runs the candidate solution under firejail with whatever ``python3`` is
        visible inside the sandbox (which strips PATH). Numpy must be available
        inside that interpreter; skip when probing through firejail itself
        reports it missing rather than fail spuriously on hosts without a
        numpy-equipped sandbox interpreter.
        """
        import shutil
        import subprocess
        ...
        if shutil.which("firejail") is None:
            ...
            pytest.skip("firejail not installed; sandboxed reward eval unavailable")
        probe = subprocess.run(
            ["firejail", "--quiet", "--noprofile", "python3", "-c", "import numpy"],
            capture_output=True,
        )
        if probe.returncode != 0:
            import pytest

            pytest.skip("sandbox python3 lacks numpy; not a code-reward bug")
        model_response = """
```python
